...
The Refinement phase is the critical bridge between Discovery (understanding what to build) and Delivery (building it). This phase produces a mini-PRD (Product Requirements Document) that serves as the single source of truth for implementation.
...
The Refinement Orchestrator will:
...
Discovery's entity model is a sketch — entities, relationships, and indicative capabilities. Refinement does the detailed work:
- Three Amigos analysis may reveal new capabilities Discovery missed
- API contracts, BDD scenarios, and Storybook stories are Refinement's job
- Discovery's sketch provides helpful context but doesn't constrain Refinement
...
- No spec changes without returning to Refinement
