---
name: af-work-management-expertise
description: Use when managing work state, Linear issues, task context, or recovering from session/compaction boundaries. Covers current-task.md lifecycle, Linear operations (linearis CLI), plans integration, TodoWrite tracking, session recovery, and issue state transitions.
...
**Reference:** Load `af-linear-api-expertise` skill for GraphQL operations
...
For GraphQL API (advanced operations like history, relations), see `af-linear-api-expertise`.
...
**Note:** For state UUIDs, query your team's workflow states first. See `af-linear-api-expertise` for helper scripts.
